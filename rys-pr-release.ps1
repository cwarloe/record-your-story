...
# Prepend a release section if not already present
...
"@
  $newContent = $cl -replace "## \[Unreleased\]\s*","## [Unreleased]\r\n\r\n$releaseNotes"
  if ($newContent -eq $cl) {
    $newContent = $releaseNotes + $cl
  }
...
$prBody = "This PR prepares the release for **$ReleaseTitle ($Version)**.\n\n- Updates CHANGELOG.md with date and bullets\n- Targets base branch `$BaseBranch`"
